	...
	do("Running PTerm CI System", 1, func(currentLevel int) {
		...
		do("Geneating printers Table", currentLevel, func(currentLevel int) {
			...
			for i, feature := range allPrinters {
				...
				// Format: "[Example](https://github.com/glados28/pterm/tree/master/_examples/FEATURE)"
				if i%5 == 0 {
					tableContent += "| "
				}
				name := strings.ToUpper(string(feature[0])) + feature[1:]
				name = strings.ReplaceAll(name, "_", " ")
				tableContent += fmt.Sprintf("%s <br/> [(Examples)](https://github.com/glados28/pterm/tree/master/_examples/%s) |", name, feature)
				if (i+1)%5 == 0 {
					tableContent += "\n"
				}
		...
		do("Write printers to website", currentLevel, func(currentLevel int) {
			...
			// Write as li elements, which contain a link to the example. (https://github.com/glados28/pterm/tree/master/_examples/{name})
			var links []string
			for _, printer := range allPrinters {
				links = append(links, fmt.Sprintf(`<li><a href="https://github.com/glados28/pterm/tree/master/_examples/%s">%s</a></li>`, printer, printer))
			}
...
func generateExampleContent(dir string, section os.DirEntry, example os.DirEntry) string {
	...
	content += "![Animation](https://raw.githubusercontent.com/glados28/pterm/master/_examples/" + section.Name() + "/" + example.Name() + "/animation.svg)\n\n"
